...
def create_ribbon():
    ...
    - Starts at CENTER top (phase_offset = pi → sin=0 center, cos=-1 front)
    - 1.875 turns — ends at BOTTOM RIGHT-FRONT
    - Extends below grid into blob area
    ...
    # Start at center, toward viewer
    phase_offset = math.pi

    # Ribbon extends from top of grid down into the blob (z=-3)
    z_top = HEIGHT
    z_bottom = -3.0
    ...
    for i in range(steps + 1):
        ...
        x = cx + amp_x * math.sin(angle)
        y = cy + amp_y * math.cos(angle)
...
def create_drip_blob():
    """TALL organic blob — vertical mound, not flat puddle."""
